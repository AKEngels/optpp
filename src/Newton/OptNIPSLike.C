...
void OptNIPSLike::setMeritFcn(MeritFcn option)
{
  ...
  if (mfcn == ArgaezTapia)
  {
    setCenteringParameter(0.2e0);
    setStepLengthToBdry(0.99995);
  }
  else if (mfcn == NormFmu)
  {
    setCenteringParameter(0.2e0);
    setStepLengthToBdry(0.8);
  }
  else if (mfcn == VanShanno)
  {
    setCenteringParameter(0.1e0);
    setStepLengthToBdry(0.95);
  }
}
...
int OptNIPSLike::checkConvg() // check convergence
{
  ...
  if (me > 0)
    xnorm += Norm2(y);
  if (mi > 0)
    xnorm += (Norm2(z) + Norm2(s));
  rftol = ftol * (1.0 + xnorm);
  if (error <= rftol)
  {
    strcpy(mesg, "L2-Norm  tolerance test passed");
    *optout << "\nL2-Norm = " << e(error, 12, 4) << "  "
            << "ftol = " << e(ftol, 12, 4) << "  (rftol = " << e(rftol, 12, 4) << ")" << endl;
    convg_status = 2;
  }
  else
  {
 ...
 ***************************************************************************/
  ...
  while (iter < MAXITER)
  {
    ...
    if (costplus <= cost + dirder_ * lstol * lambda)
    {
      ...
      backtracks += iter - 1;
      if (iter == 1)
        return (Newton_Step); // Newton Step
      else
        return (Backtrack_Step); // Backtrack Step
    }
 ...
 */
  ...
  if (debug_)
    *optout << "OptNIPSLike::ReadOptInput: Reading opt.input file\n";
